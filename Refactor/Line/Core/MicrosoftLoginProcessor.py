...
class MicrosoftLoginHandler():
    ...
    def get_access_token(self):
        ...
        if(res.text == ""):
            ...
            self.IOController = GlobalIOController.GlobalIOController(UserInformation, os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "\\Data")
...
class MultiThreadTokenRefresher(QThread):
    ...
    def run(self):
        self.IOController = GlobalIOController.GlobalIOController(None, os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "\\Data")
        self.RefreshToken = self.IOController.read_refresh_token(self.User)
        if self.RefreshToken != "NotFound":
            self.Handler = MicrosoftLoginHandler(None, self.RefreshToken)
            self.AccessToken.emit(self.Handler.refresh_access_token())
        else:
            self.AccessToken.emit("NotFound")